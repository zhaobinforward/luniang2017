...
//语言函数
function lang($langvar = null, $file = 'message', $vars = array(), $default = null) {
	global $_G;
	...
	if(!isset($_G['lang'][$key])) {
		include APP_ROOT.'/language/'.($path == '' ? '' : $path.'/').'lang_'.$file.'.php';
...
function hiddenstr($string, $hidlen, $replace = '*', $maxlen = 0, $charset = 'uft-8') {
	...
	if($maxlen) {
		$string = cutstr($string, $maxlen, '', 1, $charset);
	}
	
	if($hidlen) {
		$wordlength = wordlength($string, 1, $charset);
		if($wordlength > $hidlen) {
			$string = cutstr($string, ($wordlength - $hidlen), '', 1, $charset);
			for($i = 0; $i < $hidlen; $i++) {
				$string .= $replace;
			}
		}
...
function cache_write($file, $array, $dir = '') {
	...
	$cachefile = ($dir ? $dir : APP_CACHE).'/'.$file;
 ...
 *	@return String
 */
function parse_insert_sql($table, $data, $replace = false) {
	$sql_head = $sql = $comma = '';
	if(is_array($data) && $data) {
 ...
 *	@param &$info Array curl_getinfo()函数返回的关联数组
 *	@param $opts Array curl的选项
		array(
			'method' => get/post,
			'cookie' => false或cookie文件绝对路径
			'headers' => array(),//一个用来设置HTTP头字段的数组。使用如下的形式的数组进行设置： array('Content-type: text/plain', 'Content-length: 100')
			'curlopts' => array(),//设置curl的options参数 如：array('CURLOPT_TIMEOUT'=>3,..)
		)
 *	@return Integer
 *		1: 请求成功
 *		0: url为空
 *		-1: 初始化curl失败
 *		-2: curl_exec失败
 *		-3: 响应的状态码有误(不等于200或302或301)
 */
function curl_http($url, $params, &$resp, &$info, $opts = array()) {
	if(strlen($url) < 1)
		return 0;
	if(isset($opts['curlopts']) && is_array($opts['curlopts'])) {
		foreach($opts['curlopts'] as $key => $value) {
			unset($opts['curlopts'][$key]);
			$opts['curlopts'][strtoupper($key)] = $value;
		}
	}
	$opts = array_merge(
		array(
			'method' => 'get',
			'cookie' => false,//false或cookie文件绝对路径
			'headers' => array(),
			'curlopts' => array(
				'CURLOPT_TIMEOUT' => 3,
				'CURLOPT_AUTOREFERER' => true,
				'CURLOPT_SSL_VERIFYPEER' => false,/*不验证https证书*/
				'CURLOPT_SSL_VERIFYHOST' => false,/*不验证httpsHOST*/
				'CURLOPT_REFERER' => '',
				'CURLOPT_USERAGENT' => '',
			),
		),
	...
	if($opts['cookie'] !== false) {
		if(file_exists($opts['cookie'])) {
			@curl_setopt($curl, CURLOPT_COOKIEFILE, $opts['cookie']); //附加cookies
		}
		@curl_setopt($curl, CURLOPT_COOKIEJAR, $opts['cookie']);//存储cookies
	}
	if($method != 'get') {
